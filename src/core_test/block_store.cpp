...
TEST (block_store, add_item)
{
	...
	germ::block_store store (init, germ::unique_path ());
	ASSERT_TRUE (!init);
	germ::open_block block (0, 1, 0, germ::keypair ().prv, 0, 0);
	germ::uint256_union hash1 (block.hash ());
...
TEST (block_store, add_nonempty_block)
{
	...
	germ::open_block block (0, 1, 0, germ::keypair ().prv, 0, 0);
	germ::uint256_union hash1 (block.hash ());
	block.signature = germ::sign_message (key1.prv, key1.pub, hash1);
	germ::transaction transaction (store.environment, nullptr, true);
	auto latest1 (store.block_get (transaction, hash1));
	ASSERT_EQ (nullptr, latest1);
	store.block_put (transaction, hash1, block);
...
TEST (block_store, add_two_items)
{
	...
	ASSERT_TRUE (!init);
	germ::keypair key1;
	germ::open_block block (0, 1, 1, germ::keypair ().prv, 0, 0);
	germ::uint256_union hash1 (block.hash ());
	block.signature = germ::sign_message (key1.prv, key1.pub, hash1);
	germ::transaction transaction (store.environment, nullptr, true);
	auto latest1 (store.block_get (transaction, hash1));
	ASSERT_EQ (nullptr, latest1);
	germ::open_block block2 (0, 1, 3, germ::keypair ().prv, 0, 0);
	block2.hashables.account = 3;
	germ::uint256_union hash2 (block2.hash ());
	block2.signature = germ::sign_message (key1.prv, key1.pub, hash2);
...
TEST (block_store, add_receive)
{
	...
	germ::receive_block block (block1.hash (), 1, germ::keypair ().prv, 2, 3);
	germ::block_hash hash1 (block.hash ());
	auto latest1 (store.block_get (transaction, hash1));
	ASSERT_EQ (nullptr, latest1);
	store.block_put (transaction, hash1, block);
	auto latest2 (store.block_get (transaction, hash1));
...
TEST (block_store, pending_iterator)
{
	...
	store.pending_put (transaction, germ::pending_key (1, 2), { 2, 3 });
	auto current (store.pending_begin (transaction));
	ASSERT_NE (store.pending_end (), current);
	germ::pending_key key1 (current->first);
	ASSERT_EQ (germ::account (1), key1.account);
	ASSERT_EQ (germ::block_hash (2), key1.hash);
	germ::pending_info pending (current->second);
	ASSERT_EQ (germ::account (2), pending.source);
	ASSERT_EQ (germ::amount (3), pending.amount);
}
...
TEST (block_store, genesis)
{
	...
	auto receive1 (dynamic_cast<germ::open_block *> (block1.get ()));
	ASSERT_NE (nullptr, receive1);
	ASSERT_LE (info.modified, germ::seconds_since_epoch ());
	auto test_pub_text (germ::test_genesis_key.pub.to_string ());
	auto test_pub_account (germ::test_genesis_key.pub.to_account ());
	auto test_prv_text (germ::test_genesis_key.prv.data.to_string ());
	ASSERT_EQ (germ::genesis_account, germ::test_genesis_key.pub);
}
...
TEST (unchecked, multiple)
{
	...
	germ::block_store store (init, germ::unique_path ());
	ASSERT_TRUE (!init);
	auto block1 (std::make_shared<germ::send_block> (4, 1, 2, germ::keypair ().prv, 4, 5));
	germ::transaction transaction (store.environment, nullptr, true);
	auto block2 (store.unchecked_get (transaction, block1->previous ()));
	ASSERT_TRUE (block2.empty ());
	store.unchecked_put (transaction, block1->previous (), block1);
...
TEST (block_store, one_bootstrap)
{
	...
	auto block2 (germ::deserialize_block (begin->second));
...
TEST (block_store, unchecked_begin_search)
{
	...
	ASSERT_TRUE (!init);
	germ::keypair key0;
	germ::send_block block1 (0, 1, 2, key0.prv, key0.pub, 3);
	germ::send_block block2 (5, 6, 7, key0.prv, key0.pub, 8);
}
...
TEST (block_store, frontier_retrieval)
{
	...
	germ::block_store store (init, germ::unique_path ());
	ASSERT_TRUE (!init);
	germ::account account1 (0);
	germ::account_info info1 (0, 0, 0, 0, 0, 0);
	germ::transaction transaction (store.environment, nullptr, true);
...
TEST (block_store, one_account)
{
	...
	germ::account_info info (begin->second);
	ASSERT_EQ (hash, info.head);
...
TEST (block_store, two_account)
{
	...
	germ::block_store store (init, germ::unique_path ());
	ASSERT_TRUE (!init);
	germ::account account1 (1);
	germ::block_hash hash1 (2);
	germ::account account2 (3);
	germ::block_hash hash2 (4);
	germ::transaction transaction (store.environment, nullptr, true);
	...
	germ::account_info info1 (begin->second);
	...
	germ::account_info info2 (begin->second);
...
TEST (block_store, DISABLED_already_open) // File can be shared
{
	...
	bool init (false);
	germ::block_store store (init, path);
	ASSERT_TRUE (init);
}
...
TEST (block_store, roots)
{
	...
	germ::block_store store (init, germ::unique_path ());
	ASSERT_TRUE (!init);
	germ::send_block send_block (0, 1, 2, germ::keypair ().prv, 4, 5);
	ASSERT_EQ (send_block.hashables.previous, send_block.root ());
	germ::change_block change_block (0, 1, germ::keypair ().prv, 3, 4);
	ASSERT_EQ (change_block.hashables.previous, change_block.root ());
	germ::receive_block receive_block (0, 1, germ::keypair ().prv, 3, 4);
	ASSERT_EQ (receive_block.hashables.previous, receive_block.root ());
	germ::open_block open_block (0, 1, 2, germ::keypair ().prv, 4, 5);
...
TEST (block_store, latest_exists)
{
	...
	germ::block_store store (init, germ::unique_path ());
	ASSERT_TRUE (!init);
	germ::block_hash two (2);
	germ::account_info info;
	germ::transaction transaction (store.environment, nullptr, true);
	store.account_put (transaction, two, info);
	germ::block_hash one (1);
...
TEST (block_store, large_iteration)
{
	...
	germ::block_store store (init, germ::unique_path ());
	ASSERT_TRUE (!init);
	std::unordered_set<germ::account> accounts1;
	for (auto i (0); i < 1000; ++i)
	{
		germ::transaction transaction (store.environment, nullptr, true);
		germ::account account;
		germ::random_pool.GenerateBlock (account.bytes.data (), account.bytes.size ());
		accounts1.insert (account);
		store.account_put (transaction, account, germ::account_info ());
	}
	std::unordered_set<germ::account> accounts2;
	germ::account previous (0);
	germ::transaction transaction (store.environment, nullptr, false);
	for (auto i (store.latest_begin (transaction, 0)), n (store.latest_end ()); i != n; ++i)
	{
		germ::account current (i->first.uint256 ());
...
TEST (block_store, frontier)
{
	...
	ASSERT_TRUE (!init);
	germ::transaction transaction (store.environment, nullptr, true);
	germ::block_hash hash (100);
	germ::account account (200);
...
TEST (block_store, block_replace)
{
	...
	germ::block_store store (init, germ::unique_path ());
	ASSERT_TRUE (!init);
	germ::send_block send1 (0, 0, 0, germ::keypair ().prv, 0, 1);
	germ::send_block send2 (0, 0, 0, germ::keypair ().prv, 0, 2);
	germ::transaction transaction (store.environment, nullptr, true);
...
TEST (block_store, block_count)
{
	...
	germ::block_store store (init, germ::unique_path ());
	ASSERT_TRUE (!init);
	ASSERT_EQ (0, store.block_count (germ::transaction (store.environment, nullptr, false)).sum ());
	germ::open_block block (0, 1, 0, germ::keypair ().prv, 0, 0);
	germ::uint256_union hash1 (block.hash ());
	store.block_put (germ::transaction (store.environment, nullptr, true), hash1, block);
	ASSERT_EQ (1, store.block_count (germ::transaction (store.environment, nullptr, false)).sum ());
}
...
TEST (block_store, account_count)
{
	...
	germ::block_store store (init, germ::unique_path ());
	ASSERT_TRUE (!init);
	ASSERT_EQ (0, store.account_count (germ::transaction (store.environment, nullptr, false)));
	germ::account account (200);
	store.account_put (germ::transaction (store.environment, nullptr, true), account, germ::account_info ());
	ASSERT_EQ (1, store.account_count (germ::transaction (store.environment, nullptr, false)));
}
...
TEST (block_store, sequence_increment)
{
	...
	germ::keypair key1;
	germ::keypair key2;
	auto block1 (std::make_shared<germ::open_block> (0, 1, 0, germ::keypair ().prv, 0, 0));
	germ::transaction transaction (store.environment, nullptr, true);
...
TEST (block_store, upgrade_v2_v3)
{
	...
	auto path (germ::unique_path ());
	{
		...
		germ::change_block change (hash, key1.pub, germ::test_genesis_key.prv, germ::test_genesis_key.pub, 0);
		...
		ASSERT_EQ (germ::genesis_amount, ledger.weight (transaction, key1.pub));
		...
		germ::account_info info;
		ASSERT_FALSE (store.account_get (transaction, germ::test_genesis_key.pub, info));
		info.rep_block = 42;
		germ::account_info_v5 info_old (info.head, info.rep_block, info.open_block, info.balance, info.modified);
		auto status (mdb_put (transaction, store.accounts, germ::mdb_val (germ::test_genesis_key.pub), info_old.val (), 0));
		assert (status == 0);
	}
	bool init (false);
	germ::block_store store (init, path);
...
TEST (block_store, upgrade_v3_v4)
{
	...
	auto path (germ::unique_path ());
	{
		...
		germ::pending_info_v3 info (key1.pub, 100, key2.pub);
		auto status (mdb_put (transaction, store.pending, germ::mdb_val (key3.pub), info.val (), 0));
		ASSERT_EQ (0, status);
	}
	bool init (false);
	germ::block_store store (init, path);
...
TEST (block_store, upgrade_v4_v5)
{
	...
	auto path (germ::unique_path ());
	{
		...
		germ::account_info info;
		store.account_get (transaction, germ::test_genesis_key.pub, info);
		germ::keypair key0;
		germ::send_block block0 (info.head, key0.pub, germ::genesis_amount - germ::Gxrb_ratio, germ::test_genesis_key.prv, germ::test_genesis_key.pub, 0);
		ASSERT_EQ (germ::process_result::progress, ledger.process (transaction, block0).code);
		hash = block0.hash ();
		...
		germ::account_info info2;
		store.account_get (transaction, germ::test_genesis_key.pub, info2);
		germ::account_info_v5 info_old (info2.head, info2.rep_block, info2.open_block, info2.balance, info2.modified);
		auto status (mdb_put (transaction, store.accounts, germ::mdb_val (germ::test_genesis_key.pub), info_old.val (), 0));
		assert (status == 0);
	}
	bool init (false);
...
TEST (block_store, upgrade_v6_v7)
{
	auto path (germ::unique_path ());
	{
		...
		auto send1 (std::make_shared<germ::send_block> (0, 0, 0, germ::test_genesis_key.prv, germ::test_genesis_key.pub, 0));
...
TEST (block_store, change_dupsort)
{
	...
	auto send1 (std::make_shared<germ::send_block> (0, 0, 0, germ::test_genesis_key.prv, germ::test_genesis_key.pub, 0));
	auto send2 (std::make_shared<germ::send_block> (1, 0, 0, germ::test_genesis_key.prv, germ::test_genesis_key.pub, 0));
...
TEST (block_store, upgrade_v8_v9)
{
	...
	germ::keypair key;
	{
		...
		ASSERT_EQ (0, mdb_put (transaction, store.vote, germ::mdb_val (key.pub), germ::mdb_val (sizeof (sequence), &sequence), 0));
...
TEST (block_store, upgrade_v9_v10)
{
	...
	germ::block_hash hash (0);
	{
		...
		{
			balance = balance - germ::Gxrb_ratio;
			germ::send_block block0 (hash, key0.pub, balance, germ::test_genesis_key.prv, germ::test_genesis_key.pub, 0);
			ASSERT_EQ (germ::process_result::progress, ledger.process (transaction, block0).code);
			hash = block0.hash ();
		}
		germ::block_info block_info_auto; // Checking automatic block_info creation for block 32
		store.block_info_get (transaction, hash, block_info_auto);
		ASSERT_EQ (block_info_auto.account, germ::test_genesis_key.pub);
	...
	ASSERT_EQ (block_info.account, germ::test_genesis_key.pub);
	ASSERT_EQ (block_info.balance.number (), germ::genesis_amount - germ::Gxrb_ratio * 31);
}

TEST (block_store, state_block)
{
	bool error (false);
	germ::block_store store (error, germ::unique_path ());
	ASSERT_FALSE (error);
	germ::genesis genesis;
	germ::transaction transaction (store.environment, nullptr, true);
	genesis.initialize (transaction, store);
	germ::keypair key1;
	germ::state_block block1 (1, genesis.hash (), 3, 4, 6, key1.prv, key1.pub, 7);
	ASSERT_EQ (germ::block_type::state, block1.type ());
	store.block_put (transaction, block1.hash (), block1);
	ASSERT_TRUE (store.block_exists (transaction, block1.hash ()));
