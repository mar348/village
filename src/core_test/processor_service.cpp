...
TEST (processor_service, bad_send_signature)
{
	...
	germ::keypair key2;
	germ::send_block send (info1.head, germ::test_genesis_key.pub, 50, germ::test_genesis_key.prv, germ::test_genesis_key.pub, 0);
	germ::block_hash hash1 (send.hash ());
	send.signature.bytes[32] ^= 0x1;
	ASSERT_EQ (germ::process_result::bad_signature, ledger.process (transaction, send).code);
}
...
TEST (processor_service, bad_receive_signature)
{
	...
	germ::receive_block receive (hash1, hash1, germ::test_genesis_key.prv, germ::test_genesis_key.pub, 0);
	receive.signature.bytes[32] ^= 0x1;
	ASSERT_EQ (germ::process_result::bad_signature, ledger.process (transaction, receive).code);
}
