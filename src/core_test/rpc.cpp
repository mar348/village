...
{
public:
	test_response (boost::property_tree::ptree const & request_a, germ::rpc & rpc_a, boost::asio::io_service & service_a) :
	request (request_a),
	sock (service_a),
	status (0)
	{
		sock.async_connect (germ::tcp_endpoint (boost::asio::ip::address_v6::loopback (), rpc_a.config.port), [this](boost::system::error_code const & ec) {
			if (!ec)
			{
...
TEST (rpc, account_balance)
{
	...
	germ::rpc rpc (system.service, *system.nodes[0], germ::rpc_config (true));
	rpc.start ();
	boost::property_tree::ptree request;
	request.put ("action", "account_balance");
	request.put ("account", germ::test_genesis_key.pub.to_account ());
...
TEST (rpc, account_weight)
{
	...
	germ::change_block block (latest, key.pub, germ::test_genesis_key.prv, germ::test_genesis_key.pub, node1.work_generate_blocking (latest));
	ASSERT_EQ (germ::process_result::progress, node1.process (block).code);
	germ::rpc rpc (system.service, node1, germ::rpc_config (true));
	rpc.start ();
	boost::property_tree::ptree request;
...
TEST (rpc, validate_account_invalid)
{
	...
	rpc.start ();
	std::string account;
	germ::test_genesis_key.pub.encode_account (account);
	account[0] ^= 0x1;
	system.wallet (0)->insert_adhoc (germ::test_genesis_key.prv);
	boost::property_tree::ptree request;
	request.put ("action", "validate_account_number");
...
TEST (rpc, send)
{
	...
	request.put ("source", germ::test_genesis_key.pub.to_account ());
	request.put ("destination", germ::test_genesis_key.pub.to_account ());
	...
	ASSERT_EQ (system.nodes[0]->latest (germ::test_genesis_key.pub), block);
	thread2.join ();
}
...
TEST (rpc, send_work)
{
	...
	request.erase ("work");
	request.put ("work", germ::to_string_hex (system.nodes[0]->work_generate_blocking (system.nodes[0]->latest (germ::test_genesis_key.pub))));
...
TEST (rpc, send_idempotent)
{
	...
	ASSERT_FALSE (block.decode_hex (block_text));
	ASSERT_TRUE (system.nodes[0]->ledger.block_exists (block));
	ASSERT_EQ (system.nodes[0]->balance (germ::test_genesis_key.pub), germ::genesis_amount - 100);
	test_response response2 (request, rpc, system.service);
	while (response2.status == 0)
	{
		system.poll ();
...
TEST (rpc, wallet_representative)
{
	...
	ASSERT_EQ (account_text1, germ::genesis_account.to_account ());
}
...
TEST (rpc, wallet_export)
{
	...
	germ::kdf kdf;
	germ::wallet_store store (error, kdf, transaction, germ::genesis_account, 1, "0", wallet_json);
	ASSERT_FALSE (error);
	ASSERT_TRUE (store.exists (transaction, germ::test_genesis_key.pub));
}
...
TEST (rpc, account_move)
{
	...
	germ::keypair key;
	destination->insert_adhoc (germ::test_genesis_key.prv);
	germ::keypair source_id;
	...
	ASSERT_TRUE (destination->exists (germ::test_genesis_key.pub));
	germ::transaction transaction (system.nodes[0]->store.environment, nullptr, false);
...
TEST (rpc, block)
{
	...
	request.put ("hash", system.nodes[0]->latest (germ::genesis_account).to_string ());
	test_response response (request, rpc, system.service);
	while (response.status == 0)
	{
		system.poll ();
...
TEST (rpc, chain)
{
	...
	system.wallet (0)->insert_adhoc (germ::test_genesis_key.prv);
	germ::keypair key;
	auto genesis (system.nodes[0]->latest (germ::test_genesis_key.pub));
	ASSERT_FALSE (genesis.is_zero ());
	auto block (system.wallet (0)->send_action (germ::test_genesis_key.pub, key.pub, 1));
	...
	std::vector<germ::block_hash> blocks;
	for (auto i (blocks_node.begin ()), n (blocks_node.end ()); i != n; ++i)
	{
		blocks.push_back (germ::block_hash (i->second.get<std::string> ("")));
	}
...
{
	germ::system system (24000, 1);
	std::unordered_map<germ::account, germ::block_hash> source;
	{
		germ::transaction transaction (system.nodes[0]->store.environment, nullptr, true);
		for (auto i (0); i < 1000; ++i)
		{
			...
			system.nodes[0]->store.account_put (transaction, key.pub, germ::account_info (key.prv.data, 0, 0, 0, 0, 0));
		}
	}
	germ::keypair key;
	germ::rpc rpc (system.service, *system.nodes[0], germ::rpc_config (true));
	...
	request.put ("account", germ::account (0).to_account ());
	request.put ("count", std::to_string (std::numeric_limits<uint64_t>::max ()));
	test_response response (request, rpc, system.service);
	while (response.status == 0)
	{
...
TEST (rpc, history)
{
	germ::system system (24000, 1);
	system.wallet (0)->insert_adhoc (germ::test_genesis_key.prv);
	auto change (system.wallet (0)->change_action (germ::test_genesis_key.pub, germ::test_genesis_key.pub));
	ASSERT_NE (nullptr, change);
	auto send (system.wallet (0)->send_action (germ::test_genesis_key.pub, germ::test_genesis_key.pub, system.nodes[0]->config.receive_minimum.number ()));
	ASSERT_NE (nullptr, send);
	auto receive (system.wallet (0)->receive_action (static_cast<germ::send_block &> (*send), germ::test_genesis_key.pub, system.nodes[0]->config.receive_minimum.number ()));
	ASSERT_NE (nullptr, receive);
	auto node0 (system.nodes[0]);
	germ::genesis genesis;
	germ::state_block usend (germ::genesis_account, node0->latest (germ::genesis_account), germ::genesis_account, germ::genesis_amount - germ::Gxrb_ratio, germ::genesis_account, germ::test_genesis_key.prv, germ::test_genesis_key.pub, 0);
	germ::state_block ureceive (germ::genesis_account, usend.hash (), germ::genesis_account, germ::genesis_amount, usend.hash (), germ::test_genesis_key.prv, germ::test_genesis_key.pub, 0);
	germ::state_block uchange (germ::genesis_account, ureceive.hash (), germ::keypair ().pub, germ::genesis_amount, 0, germ::test_genesis_key.prv, germ::test_genesis_key.pub, 0);
	{
		germ::transaction transaction (node0->store.environment, nullptr, true);
		ASSERT_EQ (germ::process_result::progress, node0->ledger.process (transaction, usend).code);
		ASSERT_EQ (germ::process_result::progress, node0->ledger.process (transaction, ureceive).code);
		ASSERT_EQ (germ::process_result::progress, node0->ledger.process (transaction, uchange).code);
	}
	germ::rpc rpc (system.service, *node0, germ::rpc_config (true));
	rpc.start ();
	boost::property_tree::ptree request;
	...
	ASSERT_EQ (germ::test_genesis_key.pub.to_account (), std::get<1> (history_l[0]));
	ASSERT_EQ (germ::Gxrb_ratio.convert_to<std::string> (), std::get<2> (history_l[0]));
	ASSERT_EQ (5, history_l.size ());
	ASSERT_EQ ("send", std::get<0> (history_l[1]));
	ASSERT_EQ (usend.hash ().to_string (), std::get<3> (history_l[1]));
	ASSERT_EQ (germ::test_genesis_key.pub.to_account (), std::get<1> (history_l[1]));
	ASSERT_EQ (germ::Gxrb_ratio.convert_to<std::string> (), std::get<2> (history_l[1]));
	ASSERT_EQ ("receive", std::get<0> (history_l[2]));
	ASSERT_EQ (germ::test_genesis_key.pub.to_account (), std::get<1> (history_l[2]));
	...
	ASSERT_EQ (germ::test_genesis_key.pub.to_account (), std::get<1> (history_l[3]));
	...
	ASSERT_EQ (germ::test_genesis_key.pub.to_account (), std::get<1> (history_l[4]));
	ASSERT_EQ (germ::genesis_amount.convert_to<std::string> (), std::get<2> (history_l[4]));
...
TEST (rpc, process_block)
{
	germ::system system (24000, 1);
	germ::keypair key;
	auto latest (system.nodes[0]->latest (germ::test_genesis_key.pub));
	auto & node1 (*system.nodes[0]);
	germ::send_block send (latest, key.pub, 100, germ::test_genesis_key.prv, germ::test_genesis_key.pub, node1.work_generate_blocking (latest));
	germ::rpc rpc (system.service, node1, germ::rpc_config (true));
	rpc.start ();
	boost::property_tree::ptree request;
	...
	ASSERT_EQ (200, response.status);
	auto iterations (0);
	while (system.nodes[0]->latest (germ::test_genesis_key.pub) != send.hash ())
	{
		system.poll ();
		++iterations;
		ASSERT_LT (iterations, 200);
...
TEST (rpc, process_republish)
{
	...
	ASSERT_EQ (200, response.status);
	auto iterations (0);
	while (system.nodes[1]->latest (germ::test_genesis_key.pub) != send.hash ())
	{
		system.poll ();
		++iterations;
		ASSERT_LT (iterations, 200);
...
TEST (rpc, payment_begin_end)
{
	...
	auto root1 (system.nodes[0]->ledger.latest_root (germ::transaction (wallet->store.environment, nullptr, false), account));
	uint64_t work (0);
	auto iteration (0);
	ASSERT_TRUE (germ::work_validate (root1, work));
	while (germ::work_validate (root1, work))
	{
		system.poll ();
		ASSERT_FALSE (wallet->store.work_get (germ::transaction (wallet->store.environment, nullptr, false), account, work));
		++iteration;
...
TEST (rpc, payment_end_nonempty)
{
	...
	auto node1 (system.nodes[0]);
	system.wallet (0)->insert_adhoc (germ::test_genesis_key.prv);
	system.wallet (0)->init_free_accounts (germ::transaction (node1->store.environment, nullptr, false));
	...
	request1.put ("account", germ::test_genesis_key.pub.to_account ());
	test_response response1 (request1, rpc, system.service);
	while (response1.status == 0)
	{
		system.poll ();
...
TEST (rpc, payment_zero_balance)
{
	...
	germ::uint256_union account;
	ASSERT_FALSE (account.decode_account (account_text));
	ASSERT_NE (germ::test_genesis_key.pub, account);
}
...
TEST (rpc, payment_wait)
{
	...
	request1.put ("amount", germ::amount (germ::Mxrb_ratio).to_string_dec ());
	...
	system.wallet (0)->send_action (germ::test_genesis_key.pub, key.pub, germ::Mxrb_ratio);
	...
	request1.put ("amount", germ::amount (germ::Mxrb_ratio * 2).to_string_dec ());
	test_response response3 (request1, rpc, system.service);
	while (response3.status == 0)
	{
		system.poll ();
...
{
	germ::system system (24000, 2);
	system.nodes[0]->peers.insert (germ::endpoint (boost::asio::ip::address_v6::from_string ("::ffff:80.80.80.80"), 4000), germ::protocol_version);
	germ::rpc rpc (system.service, *system.nodes[0], germ::rpc_config (true));
	rpc.start ();
	boost::property_tree::ptree request;
...
TEST (rpc, pending)
{
	...
	germ::keypair key1;
	system.wallet (0)->insert_adhoc (germ::test_genesis_key.prv);
	auto block1 (system.wallet (0)->send_action (germ::test_genesis_key.pub, key1.pub, 100));
	germ::rpc rpc (system.service, *system.nodes[0], germ::rpc_config (true));
	rpc.start ();
	boost::property_tree::ptree request;
	...
	germ::block_hash hash1 (blocks_node.begin ()->second.get<std::string> (""));
	...
	ASSERT_EQ (1, blocks_node.size ());
	std::unordered_map<germ::block_hash, germ::uint128_union> blocks;
	...
	ASSERT_EQ (1, blocks_node.size ());
	std::unordered_map<germ::block_hash, germ::uint128_union> amounts;
	std::unordered_map<germ::block_hash, germ::account> sources;
	for (auto i (blocks_node.begin ()), j (blocks_node.end ()); i != j; ++i)
	{
		germ::block_hash hash;
		hash.decode_hex (i->first);
	...
	ASSERT_EQ (sources[block1->hash ()], germ::test_genesis_key.pub);
}

TEST (rpc_config, serialization)
{
	germ::rpc_config config1;
...
TEST (rpc, search_pending)
{
	...
	germ::send_block block (system.nodes[0]->latest (germ::test_genesis_key.pub), germ::test_genesis_key.pub, germ::genesis_amount - system.nodes[0]->config.receive_minimum.number (), germ::test_genesis_key.prv, germ::test_genesis_key.pub, 0);
	ASSERT_EQ (germ::process_result::progress, system.nodes[0]->ledger.process (germ::transaction (system.nodes[0]->store.environment, nullptr, true), block).code);
	germ::rpc rpc (system.service, *system.nodes[0], germ::rpc_config (true));
	rpc.start ();
	boost::property_tree::ptree request;
...
TEST (rpc, version)
{
	...
	ASSERT_EQ (boost::str (boost::format ("RaiBlocks %1%.%2%") % GERMBLOCKS_VERSION_MAJOR % GERMBLOCKS_VERSION_MINOR), response1.json.get<std::string> ("node_vendor"));
...
TEST (rpc, work_generate)
{
	...
	uint64_t work2;
	ASSERT_FALSE (germ::from_string_hex (work1, work2));
	ASSERT_FALSE (germ::work_validate (hash1, work2));
}
...
TEST (rpc, work_peer_bad)
{
	...
	while (germ::work_validate (hash1, work))
	{
		system.poll ();
	}
}
...
TEST (rpc, work_peer_one)
{
	...
	while (germ::work_validate (key1.pub, work))
	{
		system.poll ();
	}
}
...
TEST (rpc, work_peer_many)
{
	...
	germ::system system3 (24002, 1);
	germ::system system4 (24003, 1);
	germ::node_init init1;
	...
	germ::keypair key;
	germ::rpc_config config2 (true);
	config2.port += 0;
	germ::rpc rpc2 (system2.service, node2, config2);
	rpc2.start ();
	germ::rpc_config config3 (true);
	config3.port += 1;
	germ::rpc rpc3 (system3.service, node3, config3);
	rpc3.start ();
	germ::rpc_config config4 (true);
	config4.port += 2;
	germ::rpc rpc4 (system4.service, node4, config4);
	rpc4.start ();
...
TEST (rpc, available_supply)
{
	...
	auto block2 (system.wallet (0)->send_action (germ::test_genesis_key.pub, 0, 100)); // Sending to burning 0 account
	test_response response3 (request1, rpc, system.service);
	while (response3.status == 0)
	{
		system.poll ();
...
TEST (rpc, mrai_to_raw)
{
	...
	ASSERT_EQ (200, response1.status);
	ASSERT_EQ (germ::Mxrb_ratio.convert_to<std::string> (), response1.json.get<std::string> ("amount"));
}
...
TEST (rpc, mrai_from_raw)
{
	...
	request1.put ("amount", germ::Mxrb_ratio.convert_to<std::string> ());
	test_response response1 (request1, rpc, system.service);
	while (response1.status == 0)
	{
		system.poll ();
...
TEST (rpc, krai_to_raw)
{
	...
	ASSERT_EQ (200, response1.status);
	ASSERT_EQ (germ::kxrb_ratio.convert_to<std::string> (), response1.json.get<std::string> ("amount"));
}
...
TEST (rpc, krai_from_raw)
{
	...
	request1.put ("amount", germ::kxrb_ratio.convert_to<std::string> ());
	test_response response1 (request1, rpc, system.service);
	while (response1.status == 0)
	{
		system.poll ();
...
TEST (rpc, rai_to_raw)
{
	...
	ASSERT_EQ (200, response1.status);
	ASSERT_EQ (germ::xrb_ratio.convert_to<std::string> (), response1.json.get<std::string> ("amount"));
}
...
TEST (rpc, rai_from_raw)
{
	...
	request1.put ("amount", germ::xrb_ratio.convert_to<std::string> ());
	test_response response1 (request1, rpc, system.service);
	while (response1.status == 0)
	{
		system.poll ();
...
TEST (rpc, account_representative)
{
	...
	boost::property_tree::ptree request;
	std::string wallet;
	request.put ("account", germ::genesis_account.to_account ());
...
TEST (rpc, bootstrap)
{
	...
	auto latest (system1.nodes[0]->latest (germ::test_genesis_key.pub));
	germ::send_block send (latest, germ::genesis_account, 100, germ::test_genesis_key.prv, germ::test_genesis_key.pub, system1.nodes[0]->work_generate_blocking (latest));
	{
		germ::transaction transaction (system1.nodes[0]->store.environment, nullptr, true);
		ASSERT_EQ (germ::process_result::progress, system1.nodes[0]->ledger.process (transaction, send).code);
	}
	germ::rpc rpc (system0.service, *system0.nodes[0], germ::rpc_config (true));
	rpc.start ();
	boost::property_tree::ptree request;
	...
	auto iterations (0);
	while (system0.nodes[0]->latest (germ::genesis_account) != system1.nodes[0]->latest (germ::genesis_account))
	{
		system0.poll ();
		system1.poll ();
		++iterations;
...
TEST (rpc, representatives)
{
	...
	std::vector<germ::account> representatives;
	...
	ASSERT_EQ (germ::genesis_account, representatives[0]);
}
...
TEST (rpc, wallet_change_seed)
{
	...
	germ::keypair seed;
	{
		germ::transaction transaction (system0.nodes[0]->store.environment, nullptr, false);
		germ::raw_key seed0;
...
TEST (rpc, wallet_frontiers)
{
	...
	ASSERT_EQ (200, response.status);
	auto & frontiers_node (response.json.get_child ("frontiers"));
	std::vector<germ::account> frontiers;
	for (auto i (frontiers_node.begin ()), n (frontiers_node.end ()); i != n; ++i)
	{
		frontiers.push_back (germ::block_hash (i->second.get<std::string> ("")));
	}
	ASSERT_EQ (1, frontiers.size ());
	ASSERT_EQ (system0.nodes[0]->latest (germ::genesis_account), frontiers[0]);
}
...
TEST (rpc, work_validate)
{
	...
	uint64_t work2 (0);
	request.put ("work", germ::to_string_hex (work2));
	test_response response2 (request, rpc, system.service);
	while (response2.status == 0)
	{
		system.poll ();
...
TEST (rpc, republish)
{
	...
	germ::open_block open (send.hash (), key.pub, key.pub, key.prv, key.pub, node1.work_generate_blocking (key.pub));
	...
	ASSERT_EQ (200, response.status);
	auto iterations (0);
	while (system.nodes[1]->balance (germ::test_genesis_key.pub) == germ::genesis_amount)
	{
		system.poll ();
		++iterations;
		ASSERT_GT (200, iterations);
...
TEST (rpc, deterministic_key)
{
	...
	germ::account account0 (system0.wallet (0)->deterministic_insert ());
	germ::account account1 (system0.wallet (0)->deterministic_insert ());
	germ::account account2 (system0.wallet (0)->deterministic_insert ());
	germ::rpc rpc (system0.service, *system0.nodes[0], germ::rpc_config (true));
	rpc.start ();
	boost::property_tree::ptree request;
...
TEST (rpc, accounts_balances)
{
	...
	entry.put ("", germ::test_genesis_key.pub.to_account ());
	...
	for (auto & balances : response.json.get_child ("balances"))
	{
		...
		ASSERT_EQ (germ::test_genesis_key.pub.to_account (), account_text);
...
TEST (rpc, accounts_frontiers)
{
	...
	for (auto & frontiers : response.json.get_child ("frontiers"))
	{
		...
		ASSERT_EQ (system.nodes[0]->latest (germ::genesis_account), frontier_text);
	}
}
...
TEST (rpc, accounts_pending)
{
	...
	for (auto & blocks : response.json.get_child ("blocks"))
	{
		...
		germ::block_hash hash1 (blocks.second.begin ()->second.get<std::string> (""));
		ASSERT_EQ (block1->hash (), hash1);
	}
	request.put ("threshold", "100"); // Threshold test
	test_response response1 (request, rpc, system.service);
...
TEST (rpc, blocks)
{
	...
	boost::property_tree::ptree entry;
	boost::property_tree::ptree peers_l;
	entry.put ("", system.nodes[0]->latest (germ::genesis_account).to_string ());
	peers_l.push_back (std::make_pair ("", entry));
	...
	for (auto & blocks : response.json.get_child ("blocks"))
	{
		...
		ASSERT_EQ (system.nodes[0]->latest (germ::genesis_account).to_string (), hash_text);
...
TEST (rpc, wallet_balance_total)
{
	...
	germ::keypair key;
	system.wallet (0)->insert_adhoc (key.prv);
	auto send (system.wallet (0)->send_action (germ::test_genesis_key.pub, key.pub, 1));
	germ::rpc rpc (system.service, *system.nodes[0], germ::rpc_config (true));
	rpc.start ();
	boost::property_tree::ptree request;
...
TEST (rpc, wallet_balances)
{
	...
	auto send (system0.wallet (0)->send_action (germ::test_genesis_key.pub, key.pub, 1));
...
TEST (rpc, pending_exists)
{
	...
	germ::keypair key1;
	system.wallet (0)->insert_adhoc (germ::test_genesis_key.prv);
	auto hash0 (system.nodes[0]->latest (germ::genesis_account));
	auto block1 (system.wallet (0)->send_action (germ::test_genesis_key.pub, key1.pub, 100));
	germ::rpc rpc (system.service, *system.nodes[0], germ::rpc_config (true));
	rpc.start ();
	boost::property_tree::ptree request;
...
TEST (rpc, wallet_pending)
{
	...
	auto block1 (system0.wallet (0)->send_action (germ::test_genesis_key.pub, key1.pub, 100));
	germ::rpc rpc (system0.service, *system0.nodes[0], germ::rpc_config (true));
	rpc.start ();
	boost::property_tree::ptree request;
	...
	for (auto & pending : response.json.get_child ("blocks"))
	{
		std::string account_text (pending.first);
		ASSERT_EQ (key1.pub.to_account (), account_text);
		germ::block_hash hash1 (pending.second.begin ()->second.get<std::string> (""));
		ASSERT_EQ (block1->hash (), hash1);
	}
	request.put ("threshold", "100"); // Threshold test
	test_response response0 (request, rpc, system0.service);
...
TEST (rpc, work_get)
{
	...
	uint64_t work (1);
	germ::transaction transaction (system.nodes[0]->store.environment, nullptr, false);
	system.nodes[0]->wallets.items.begin ()->second->store.work_get (transaction, germ::genesis_account, work);
	ASSERT_EQ (germ::to_string_hex (work), work_text);
}
...
TEST (rpc, work_set)
{
	...
	request.put ("wallet", system.nodes[0]->wallets.items.begin ()->first.to_string ());
	request.put ("account", germ::test_genesis_key.pub.to_account ());
	request.put ("work", germ::to_string_hex (work0));
	test_response response (request, rpc, system.service);
	while (response.status == 0)
	{
		system.poll ();
	...
	uint64_t work1 (1);
	germ::transaction transaction (system.nodes[0]->store.environment, nullptr, false);
	system.nodes[0]->wallets.items.begin ()->second->store.work_get (transaction, germ::genesis_account, work1);
...
TEST (rpc, wallet_republish)
{
	...
	germ::genesis genesis;
	germ::keypair key;
	while (key.pub < germ::test_genesis_key.pub)
	{
		germ::keypair key1;
		key.pub = key1.pub;
...
TEST (rpc, delegators)
{
	...
	germ::send_block send (latest, key.pub, 100, germ::test_genesis_key.prv, germ::test_genesis_key.pub, node1.work_generate_blocking (latest));
	system.nodes[0]->process (send);
	germ::open_block open (send.hash (), germ::test_genesis_key.pub, key.pub, key.prv, key.pub, node1.work_generate_blocking (key.pub));
	ASSERT_EQ (germ::process_result::progress, system.nodes[0]->process (open).code);
	germ::rpc rpc (system.service, *system.nodes[0], germ::rpc_config (true));
	rpc.start ();
	boost::property_tree::ptree request;
	...
	ASSERT_EQ ("100", delegators.get<std::string> (germ::test_genesis_key.pub.to_account ()));
...
TEST (rpc, account_info)
{
	...
	germ::send_block send (latest, key.pub, 100, germ::test_genesis_key.prv, germ::test_genesis_key.pub, node1.work_generate_blocking (latest));
	system.nodes[0]->process (send);
	auto time (germ::seconds_since_epoch ());
	...
	ASSERT_EQ (germ::test_genesis_key.pub.to_account (), representative2);
}
...
TEST (rpc, blocks_info)
{
	...
	for (auto & blocks : response.json.get_child ("blocks"))
	{
		...
		ASSERT_EQ (germ::genesis_amount.convert_to<std::string> (), amount_text);
	...
	for (auto & blocks : response2.json.get_child ("blocks"))
	{
		...
		ASSERT_EQ (germ::genesis_amount.convert_to<std::string> (), balance_text);
	}
}
...
TEST (rpc, ledger)
{
	...
	for (auto & accounts : response2.json.get_child ("accounts"))
	{
		...
		ASSERT_EQ (germ::test_genesis_key.pub.to_account (), representative.get ());
	}
}
...
TEST (rpc, block_create)
{
	...
	germ::send_block send (latest, key.pub, 100, germ::test_genesis_key.prv, germ::test_genesis_key.pub, send_work);
	auto open_work = node1.work_generate_blocking (key.pub);
	germ::open_block open (send.hash (), germ::test_genesis_key.pub, key.pub, key.prv, key.pub, open_work);
	germ::rpc rpc (system.service, *system.nodes[0], germ::rpc_config (true));
	rpc.start ();
	boost::property_tree::ptree request;
	...
	request.put ("work", germ::to_string_hex (send_work));
	test_response response (request, rpc, system.service);
	while (response.status == 0)
	{
		system.poll ();
	...
	auto send_block (germ::deserialize_block_json (block_l));
	...
	request1.put ("representative", germ::test_genesis_key.pub.to_account ());
	request1.put ("source", send.hash ().to_string ());
	request1.put ("work", germ::to_string_hex (open_work));
	test_response response1 (request1, rpc, system.service);
	while (response1.status == 0)
	{
		system.poll ();
	...
	auto open_block (germ::deserialize_block_json (block_l));
	...
	germ::change_block change (open.hash (), key.pub, key.prv, key.pub, change_work);
	request1.put ("type", "change");
	request1.put ("work", germ::to_string_hex (change_work));
	...
	auto change_block (germ::deserialize_block_json (block_l));
	ASSERT_EQ (change.hash (), change_block->hash ());
	ASSERT_EQ (germ::process_result::progress, node1.process (change).code);
	germ::send_block send2 (send.hash (), key.pub, 0, germ::test_genesis_key.prv, germ::test_genesis_key.pub, node1.work_generate_blocking (send.hash ()));
	...
	request2.put ("work", germ::to_string_hex (node1.work_generate_blocking (change.hash ())));
	...
	auto receive_block (germ::deserialize_block_json (block_l));
...
TEST (rpc, block_create_state)
{
	...
	request.put ("representative", germ::test_genesis_key.pub.to_account ());
	request.put ("balance", (germ::genesis_amount - germ::Gxrb_ratio).convert_to<std::string> ());
	request.put ("link", key.pub.to_account ());
	request.put ("work", germ::to_string_hex (system.nodes[0]->work_generate_blocking (genesis.hash ())));
	germ::rpc rpc (system.service, *system.nodes[0], germ::rpc_config (true));
	rpc.start ();
	test_response response (request, rpc, system.service);
	while (response.status == 0)
	...
	boost::property_tree::ptree block_l;
	boost::property_tree::read_json (block_stream, block_l);
	auto state_block (germ::deserialize_block_json (block_l));
	ASSERT_NE (nullptr, state_block);
	ASSERT_EQ (germ::block_type::state, state_block->type ());
	ASSERT_EQ (state_hash, state_block->hash ().to_string ());
	auto process_result (system.nodes[0]->process (*state_block));
	ASSERT_EQ (germ::process_result::progress, process_result.code);
}
...
TEST (rpc, block_create_state_open)
{
	...
	germ::genesis genesis;
	system.wallet (0)->insert_adhoc (germ::test_genesis_key.prv);
	auto send_block (system.wallet (0)->send_action (germ::test_genesis_key.pub, key.pub, germ::Gxrb_ratio));
	...
	request.put ("representative", germ::test_genesis_key.pub.to_account ());
	request.put ("balance", germ::Gxrb_ratio.convert_to<std::string> ());
	request.put ("link", send_block->hash ().to_string ());
	request.put ("work", germ::to_string_hex (system.nodes[0]->work_generate_blocking (send_block->hash ())));
	germ::rpc rpc (system.service, *system.nodes[0], germ::rpc_config (true));
	rpc.start ();
	test_response response (request, rpc, system.service);
	while (response.status == 0)
...
TEST (rpc, wallet_add_watch)
{
	...
	std::string success (response.json.get<std::string> ("success"));
	ASSERT_TRUE (success.empty ());
	ASSERT_TRUE (system.wallet (0)->exists (germ::test_genesis_key.pub));
}
...
TEST (rpc, online_reps)
{
	...
	germ::rpc rpc (system.service, *system.nodes[1], germ::rpc_config (true));
	rpc.start ();
	boost::property_tree::ptree request;
	...
	ASSERT_EQ (germ::test_genesis_key.pub.to_account (), item->first);
	system.nodes[1]->stop ();
}
...
TEST (rpc, confirmation_history)
{
	germ::system system (24000, 1);
	system.wallet (0)->insert_adhoc (germ::test_genesis_key.prv);
	auto block (system.wallet (0)->send_action (germ::test_genesis_key.pub, germ::test_genesis_key.pub, germ::Gxrb_ratio));
	auto iterations (0);
	...
	ASSERT_EQ ((germ::genesis_amount - germ::Gxrb_ratio).convert_to<std::string> (), tally);
	system.stop ();
}
