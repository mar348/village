...
{
	auto file (germ::unique_path ());
	germ::account account (1);
	germ::open_block open (1, 2, 3, nullptr);
	germ::account_info_v1 v1 (open.hash (), open.hash (), 3, 4);
	{
		auto error (false);
		germ::block_store store (error, file);
		ASSERT_FALSE (error);
		germ::transaction transaction (store.environment, nullptr, true);
		store.block_put (transaction, open.hash (), open);
		auto status (mdb_put (transaction, store.accounts, germ::mdb_val (account), v1.val (), 0));
		ASSERT_EQ (0, status);
