...
void rai_daemon::daemon::run (boost::filesystem::path const & data_path)
{
    ...
    std::unique_ptr<germ::thread_runner> runner;
    ...
    germ::work_pool opencl_work (config.node.work_threads, opencl ? [&opencl](germ::uint256_union const & root_a) {
    ...
    try
    {
        auto node (std::make_shared<germ::node> (init, service, data_path, alarm, config.node, opencl_work));
        if (init.error ())
        {
        ...
        runner = std::make_unique<germ::thread_runner> (service, node->config.io_threads);
        runner->join ();
    }
