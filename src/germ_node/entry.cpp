...
#include <src/germ_node/daemon.hpp>
...
int main (int argc, char * const * argv)
{
    ...
    germ::add_node_options (description);
    ...
    if (!germ::handle_node_options (vm))
    {
    }
    ...
    {
        germ::inactive_node node (data_path);
    ...
    else if (vm.count ("debug_bootstrap_generate"))
    {
        ...
        germ::keypair genesis (key.to_string ());
        ...
        germ::uint128_t balance (std::numeric_limits<germ::uint128_t>::max ());
        germ::open_block genesis_block (genesis.pub, /*genesis.pub,*/ genesis.pub, genesis.prv, genesis.pub, work.generate (genesis.pub));
        std::cout << genesis_block.to_json ();
        germ::block_hash previous (genesis_block.hash ());
        for (auto i (0); i != 8; ++i)
        {
            germ::uint128_t yearly_distribution (germ::uint128_t (1) << (127 - (i == 7 ? 6 : i)));
            ...
            for (auto j (0); j != 52; ++j)
            {
                ...
                germ::send_block send (previous, landing.pub, balance, genesis.prv, genesis.pub, work.generate (previous));
                previous = send.hash ();
...
//        }
//        std::map<germ::account, germ::uint128_t> calculated;
//        for (auto i (node.node->store.latest_begin (transaction)), n (node.node->store.latest_end ()); i != n; ++i)
//        {
//            germ::account_info info (i->second);
//            germ::block_hash rep_block (node.node->ledger.representative_calculated (transaction, info.head));
//            std::unique_ptr<germ::tx> block (node.node->store.block_get (transaction, rep_block));
...
//        }
    ...
    else if (vm.count ("debug_profile_kdf"))
    {
        ...
        germ::uint256_union salt (0);
        std::string password ("");
        for (; true;)
        {
            auto begin1 (std::chrono::high_resolution_clock::now ());
            auto success (argon2_hash (1, germ::wallet_store::kdf_work, 1, password.data (), password.size (), salt.bytes.data (), salt.bytes.size (), result.bytes.data (), result.bytes.size (), NULL, 0, Argon2_d, 0x10));
    ...
    {
        germ::work_pool work (std::numeric_limits<unsigned>::max (), nullptr);
        germ::change_block block (0, /*0,*/ germ::keypair ().prv, 0, 0);
    ...
    else if (vm.count ("debug_opencl"))
    {
        ...
        auto opencl (germ::opencl_work::create (true, { platform, device, threads }, logging));
        germ::work_pool work_pool (std::numeric_limits<unsigned>::max (), opencl ? [&opencl](germ::uint256_union const & root_a) {
            return opencl->generate_work (root_a);
        }
                                                                                : std::function<boost::optional<uint64_t> (germ::uint256_union const &)> (nullptr));
        germ::change_block block (0, /*0,*/ germ::keypair ().prv, 0, 0);
    ...
    {
        germ::keypair key;
        germ::uint256_union message;
        germ::uint512_union signature;
        signature = germ::sign_message (key.prv, key.pub, message);
        auto begin (std::chrono::high_resolution_clock::now ());
        for (auto i (0u); i < 1000; ++i)
        {
            germ::validate_message (key.pub, message, signature);
        }
    ...
    else if (vm.count ("debug_profile_sign"))
    {
        ...
        {
            germ::keypair key;
            germ::block_hash latest (0);
            auto begin1 (std::chrono::high_resolution_clock::now ());
            for (uint64_t balance (0); balance < 1000; ++balance)
            {
                germ::send_block send (latest, key.pub, balance, key.prv, key.pub, 0);
                latest = send.hash ();
            }
            auto end1 (std::chrono::high_resolution_clock::now ());
            std::cerr << boost::str (boost::format ("%|1$ 12d|\n") % std::chrono::duration_cast<std::chrono::microseconds> (end1 - begin1).count ());
    ...
    {
        std::cout << "Version " << GERMBLOCKS_VERSION_MAJOR << "." << GERMBLOCKS_VERSION_MINOR << std::endl;
    }
    else
    {
