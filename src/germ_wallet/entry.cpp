...
bool update_config (qt_wallet_config & config_a, boost::filesystem::path const & config_path_a, std::fstream & config_file_a)
{
    ...
    auto error (false);
    if (!germ::fetch_object (config_a, config_path_a, config_file_a))
    {
...
int run_wallet (QApplication & application, int argc, char * const * argv, boost::filesystem::path const & data_path)
{
    ...
    auto error (germ::fetch_object (config, config_path, config_file));
    config_file.close ();
    if (error)
    {
    ...
    std::shared_ptr<germ_qt::wallet> gui;
    germ::set_application_icon (application);
    auto opencl (germ::opencl_work::create (config.opencl_enable, config.opencl, config.node.logging));
    germ::work_pool work (config.node.work_threads, opencl ? [&opencl](germ::uint256_union const & root_a) {
        return opencl->generate_work (root_a);
    }
                                                          : std::function<boost::optional<uint64_t> (germ::uint256_union const &)> (nullptr));
    germ::alarm alarm (service);
    germ::node_init init;
    node = std::make_shared<germ::node> (init, service, data_path, alarm, config.node, work);
    if (!init.error ())
    {
        ...
        if (config.account.is_zero () || !wallet->exists (config.account))
        {
            ...
            {
                germ::uint256_union account (existing->first.uint256 ());
        ...
        node->start ();
        std::unique_ptr<germ::rpc> rpc = get_rpc (service, *node, config.rpc);
        if (rpc && config.rpc_enable)
        {
            rpc->start ();
        }
        germ::thread_runner runner (service, node->config.io_threads);
        QObject::connect (&application, &QApplication::aboutToQuit, [&]() {
            rpc->stop ();
            node->stop ();
        });
        application.postEvent (&processor, new germ_qt::eventloop_event ([&]() {
            gui = std::make_shared<germ_qt::wallet> (application, processor, *node, wallet, config.account);
            splash->close ();
            gui->start ();
