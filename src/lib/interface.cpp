...
{
    auto const & number (*reinterpret_cast<germ::uint128_union *> (source));
    strncpy (destination, number.to_string_dec ().c_str (), 32);
}

void xrb_uint256_to_string (xrb_uint256 source, char * destination)
{
    auto const & number (*reinterpret_cast<germ::uint256_union *> (source));
...
{
    auto const & number (*reinterpret_cast<germ::uint512_union *> (source));
    strncpy (destination, number.to_string ().c_str (), 128);
}

int xrb_uint128_from_dec (const char * source, xrb_uint128 destination)
{
    auto & number (*reinterpret_cast<germ::uint128_union *> (destination));
...
{
    auto & number (*reinterpret_cast<germ::uint256_union *> (destination));
...
{
    auto & number (*reinterpret_cast<germ::uint512_union *> (destination));
    auto error (number.decode_hex (source));
    return error ? 1 : 0;
}
...
{
    auto & number (*reinterpret_cast<germ::uint256_union *> (seed));
    germ::random_pool.GenerateBlock (number.bytes.data (), number.bytes.size ());
}

void xrb_seed_key (xrb_uint256 seed, int index, xrb_uint256 destination)
{
    auto & seed_l (*reinterpret_cast<germ::uint256_union *> (seed));
    auto & destination_l (*reinterpret_cast<germ::uint256_union *> (destination));
    germ::deterministic_key (seed_l, index, destination_l);
}
...
char * xrb_sign_transaction (const char * transaction, const xrb_uint256 private_key)
{
    ...
    try
    {
        ...
        germ::raw_key prv;
        prv.data = *reinterpret_cast<germ::uint256_union *> (private_key);
        block->signature_set (germ::sign_message (prv, pub, block->hash ()));
...
char * xrb_work_transaction (const char * transaction)
{
    ...
    try
    {
        ...
        germ::work_pool pool (std::thread::hardware_concurrency ());
...
{
    germ::random_pool.GenerateBlock (reinterpret_cast<uint8_t *> (out), outlen);
}
