...
germ::tcp_bulk_pull_client::tcp_bulk_pull_client (std::shared_ptr<germ::tcp_bootstrap_client> connection_a, germ::pull_info const & pull_a) :
...
germ::tcp_bulk_pull_client::~tcp_bulk_pull_client ()
{
...
void germ::tcp_bulk_pull_client::request ()
{
    expected = pull.head;
    germ::bulk_pull req;
...
void germ::tcp_bulk_pull_client::receive_block ()
{
    auto this_l (shared_from_this ());
    connection->socket->async_read (connection->receive_buffer, 1 + sizeof(size_t), [this_l](boost::system::error_code const & ec, size_t size_a) {
        if (!ec)
        {
...
void germ::tcp_bulk_pull_client::received_type ()
{
    auto this_l (shared_from_this ());
    germ::block_type type (static_cast<germ::block_type> (connection->receive_buffer->data ()[0]));
    size_t size (*reinterpret_cast<size_t*> (&connection->receive_buffer->data ()[1]));
    switch (type)
    {
        case germ::block_type::send:
        {
            connection->socket->async_read (connection->receive_buffer, size, [this_l, type](boost::system::error_code const & ec, size_t size_a) {
...
void germ::tcp_bulk_pull_client::received_block (boost::system::error_code const & ec, size_t size_a, germ::block_type type_a)
{
    std::cout << "bulk pull receive block," << ec.message() << "," << size_a << std::endl;
    if (!ec)
    {
