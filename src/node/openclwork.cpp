...
germ::opencl_environment::opencl_environment (bool & error_a)
{
...
void germ::opencl_environment::dump (std::ostream & stream)
{
    auto index (0);
    auto device_count (0);
...
germ::opencl_config::opencl_config () :
platform (0),
device (0),
threads (1024 * 1024)
{
...
germ::opencl_config::opencl_config (unsigned platform_a, unsigned device_a, unsigned threads_a) :
platform (platform_a),
device (device_a),
threads (threads_a)
{
...
void germ::opencl_config::serialize_json (boost::property_tree::ptree & tree_a) const
{
...
bool germ::opencl_config::deserialize_json (boost::property_tree::ptree const & tree_a)
{
    auto result (false);
    try
    {
...
germ::opencl_work::opencl_work (bool & error_a, germ::opencl_config const & config_a, germ::opencl_environment & environment_a, germ::logging & logging_a) :
config (config_a),
context (0),
attempt_buffer (0),
result_buffer (0),
...
logging (logging_a)
{
    ...
    germ::random_pool.GenerateBlock (reinterpret_cast<uint8_t *> (rand.s.data ()), rand.s.size () * sizeof (decltype (rand.s)::value_type));
    ...
    cl_int item_error (0);
    size_t item_size (sizeof (germ::uint256_union));
...
boost::optional<uint64_t> germ::opencl_work::generate_work (germ::uint256_union const & root_a)
{
    ...
    while (!germ::work_validate (root_a, result) && !error)
    {
        result = rand.next ();
        ...
        cl_int write_error2 = clEnqueueWriteBuffer (queue, item_buffer, false, 0, sizeof (germ::uint256_union), root_a.bytes.data (), 0, nullptr, nullptr);
...
std::unique_ptr<germ::opencl_work> germ::opencl_work::create (bool create_a, germ::opencl_config const & config_a, germ::logging & logging_a)
{
    std::unique_ptr<germ::opencl_work> result;
    if (!create_a)
        return result;
    ...
    if (!error)
    {
        result.reset (new germ::opencl_work (error, config_a, environment, logging_a));
        if (error)
        {
            result.reset ();
        }
