...
bool germ::rpc_secure::on_verify_certificate (bool preverified, boost::asio::ssl::verify_context & ctx)
{
...
void germ::rpc_secure::load_certs (boost::asio::ssl::context & context_a)
{
    ...
    if (config.secure.client_certs_path.size () > 0)
    {
        ...
        context_a.set_verify_callback (boost::bind (&germ::rpc_secure::on_verify_certificate, this, _1, _2));
    }
}

germ::rpc_secure::rpc_secure (boost::asio::io_service & service_a, germ::node & node_a, germ::rpc_config const & config_a) :
...
void germ::rpc_secure::accept ()
{
    auto connection (std::make_shared<germ::rpc_connection_secure> (node, *this));
...
germ::rpc_connection_secure::rpc_connection_secure (germ::node & node_a, germ::rpc_secure & rpc_a) :
germ::rpc_connection (node_a, rpc_a),
stream (socket, rpc_a.ssl_context)
{
}

void germ::rpc_connection_secure::parse_connection ()
{
    // Perform the SSL handshake
    stream.async_handshake (boost::asio::ssl::stream_base::server,
    std::bind (
    &germ::rpc_connection_secure::handle_handshake,
    std::static_pointer_cast<germ::rpc_connection_secure> (shared_from_this ()),
    std::placeholders::_1));
}

void germ::rpc_connection_secure::on_shutdown (const boost::system::error_code & error)
{
...
void germ::rpc_connection_secure::handle_handshake (const boost::system::error_code & error)
{
    if (!error)
    {
        read ();
...
void germ::rpc_connection_secure::read ()
{
    auto this_l (std::static_pointer_cast<germ::rpc_connection_secure> (shared_from_this ()));
            ...
            auto response_handler ([this_l, version, start](boost::property_tree::ptree const & tree_a) {
                ...
                boost::beast::http::async_write (this_l->stream, this_l->res, [this_l](boost::system::error_code const & ec, size_t bytes_transferred) {
                    ...
                    this_l->stream.async_shutdown (
                            std::bind (
                                    &germ::rpc_connection_secure::on_shutdown,
                                    this_l,
                                    std::placeholders::_1));
