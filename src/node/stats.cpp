...
bool germ::stat_config::deserialize_json (boost::property_tree::ptree & tree_a)
{
    bool error = false;
...
std::string germ::stat_log_sink::tm_to_string (tm & tm)
{
...
class json_writer : public germ::stat_log_sink
{
...
class file_writer : public germ::stat_log_sink
{
public:
    std::ofstream log;
    std::string filename;
...
germ::stat::stat (germ::stat_config config) :
config (config)
{
}

std::shared_ptr<germ::stat_entry> germ::stat::get_entry (uint32_t key)
{
    return get_entry (key, config.interval, config.capacity);
}

std::shared_ptr<germ::stat_entry> germ::stat::get_entry (uint32_t key, size_t interval, size_t capacity)
{
    std::unique_lock<std::mutex> lock (stat_mutex);
    return get_entry_impl (key, interval, capacity);
}

std::shared_ptr<germ::stat_entry> germ::stat::get_entry_impl (uint32_t key, size_t interval, size_t capacity)
{
    std::shared_ptr<germ::stat_entry> res;
    auto entry = entries.find (key);
    if (entry == entries.end ())
    {
        res = entries.insert (std::make_pair (key, std::make_shared<germ::stat_entry> (capacity, interval))).first->second;
    }
    else
    {
        res = entry->second;
...
std::unique_ptr<germ::stat_log_sink> germ::stat::log_sink_json ()
{
    return std::make_unique<json_writer> ();
}

std::unique_ptr<germ::stat_log_sink> log_sink_file (std::string filename)
{
    return std::make_unique<file_writer> (filename);
}

void germ::stat::log_counters (stat_log_sink & sink)
{
    std::unique_lock<std::mutex> lock (stat_mutex);
    log_counters_impl (sink);
}

void germ::stat::log_counters_impl (stat_log_sink & sink)
{
    sink.begin ();
...
void germ::stat::log_samples (stat_log_sink & sink)
{
    std::unique_lock<std::mutex> lock (stat_mutex);
    log_samples_impl (sink);
}

void germ::stat::log_samples_impl (stat_log_sink & sink)
{
    sink.begin ();
    if (sink.entries () >= config.log_rotation_count)
    {
...
void germ::stat::update (uint32_t key_a, uint64_t value)
{
    ...
    std::chrono::duration<double, std::milli> duration_s = now - entry->sample_start_time;
    if (duration_s.count () > entry->sample_interval)
    {
        ...
        // Log sink
        duration_s = now - log_last_sample_writeout;
        if (config.log_interval_samples > 0 && duration_s.count () > config.log_interval_samples)
        {
...
std::string germ::stat::type_to_string (uint32_t key)
{
    auto type = static_cast<stat::type> (key >> 16 & 0x000000ff);
    std::string res;
    switch (type)
    {
        case germ::stat::type::block:
            res = "block";
            break;
        case germ::stat::type::bootstrap:
            res = "bootstrap";
            break;
        case germ::stat::type::error:
            res = "error";
            break;
        case germ::stat::type::ledger:
            res = "ledger";
            break;
        case germ::stat::type::peering:
            res = "peering";
            break;
        case germ::stat::type::rollback:
            res = "rollback";
            break;
        case germ::stat::type::traffic:
            res = "traffic";
            break;
        case germ::stat::type::vote:
            res = "vote";
            break;
        case germ::stat::type::message:
            res = "message";
            break;
    }
    return res;
...
std::string germ::stat::detail_to_string (uint32_t key)
{
    auto detail = static_cast<stat::detail> (key >> 8 & 0x000000ff);
    std::string res;
    switch (detail)
    {
        case germ::stat::detail::all:
            res = "all";
            break;
        case germ::stat::detail::bad_sender:
            res = "bad_sender";
            break;
        case germ::stat::detail::bulk_pull:
            res = "bulk_pull";
            break;
        case germ::stat::detail::bulk_pull_blocks:
            res = "bulk_pull_blocks";
            break;
        case germ::stat::detail::bulk_push:
            res = "bulk_push";
            break;
        case germ::stat::detail::change:
            res = "change";
            break;
        case germ::stat::detail::confirm_ack:
            res = "confirm_ack";
            break;
        case germ::stat::detail::node_id_handshake:
            res = "node_id_handshake";
            break;
        case germ::stat::detail::confirm_req:
            res = "confirm_req";
            break;
        case germ::stat::detail::frontier_req:
            res = "frontier_req";
            break;
        case germ::stat::detail::epoch_bulk_pull:
            res = "epoch_bulk_pull";
            break;
        case germ::stat::detail::epoch_bulk_push:
            res = "epoch_bulk_push";
            break;
        case germ::stat::detail::epoch_req:
            res = "epoch_req";
            break;
        case germ::stat::detail::transaction:
            res = "transaction";
            break;
        case germ::stat::detail::vote:
            res = "vote";
            break;
        case germ::stat::detail::handshake:
            res = "handshake";
            break;
        case germ::stat::detail::initiate:
            res = "initiate";
            break;
        case germ::stat::detail::insufficient_work:
            res = "insufficient_work";
            break;
        case germ::stat::detail::keepalive:
            res = "keepalive";
            break;
        case germ::stat::detail::open:
            res = "open";
            break;
        case germ::stat::detail::publish:
            res = "publish";
            break;
        case germ::stat::detail::receive:
            res = "receive";
            break;
        case germ::stat::detail::republish_vote:
            res = "republish_vote";
            break;
        case germ::stat::detail::send:
            res = "send";
            break;
        case germ::stat::detail::state_block:
            res = "state_block";
            break;
        case germ::stat::detail::vote_valid:
            res = "vote_valid";
            break;
        case germ::stat::detail::vote_replay:
            res = "vote_replay";
            break;
        case germ::stat::detail::vote_invalid:
            res = "vote_invalid";
            break;
    }
    return res;
...
std::string germ::stat::dir_to_string (uint32_t key)
{
    auto dir = static_cast<stat::dir> (key & 0x000000ff);
    std::string res;
    switch (dir)
    {
        case germ::stat::dir::in:
            res = "in";
            break;
        case germ::stat::dir::out:
            res = "out";
            break;
    }
    return res;
