...
{
    bool error (false);
    germ::open_or_create (stream_a, path_a.string ());
    if (stream_a.fail ())
        return error;
...
public:
    ...
    mdb_val (size_t, void *);
    mdb_val (germ::uint128_union const &);
    mdb_val (germ::uint256_union const &);
    void * data () const;
    size_t size () const;
    germ::uint256_union uint256 () const;
...
{
public:
    transaction (germ::mdb_env &, MDB_txn *, bool);
    ~transaction ();
