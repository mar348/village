...
bool germ_qt::eventloop_processor::event (QEvent * event_a)
{
    assert (dynamic_cast<germ_qt::eventloop_event *> (event_a) != nullptr);
    static_cast<germ_qt::eventloop_event *> (event_a)->action ();
    return true;
}

germ_qt::eventloop_event::eventloop_event (std::function<void()> const & action_a) :
QEvent (QEvent::Type::User),
action (action_a)
{
}

germ_qt::self_pane::self_pane (germ_qt::wallet & wallet_a, germ::account const & account_a) :
window (new QWidget),
...
wallet (wallet_a)
{
    ...
    version = new QLabel (boost::str (boost::format ("Version %1%.%2%") % GERMBLOCKS_VERSION_MAJOR % GERMBLOCKS_VERSION_MINOR).c_str ());
...
void germ_qt::self_pane::refresh_balance ()
{
...
germ_qt::accounts::accounts (germ_qt::wallet & wallet_a) :
window (new QWidget),
...
wallet (wallet_a)
{
    ...
    QObject::connect (create_account, &QPushButton::released, [this]() {
        germ::transaction transaction (this->wallet.wallet_m->store.environment, nullptr, true);
    ...
    QObject::connect (backup_seed, &QPushButton::released, [this]() {
        germ::raw_key seed;
        germ::transaction transaction (this->wallet.wallet_m->store.environment, nullptr, false);
        if (this->wallet.wallet_m->store.valid_password (transaction))
        {
...
void germ_qt::accounts::refresh_wallet_balance ()
{
    germ::transaction transaction (this->wallet.wallet_m->store.environment, nullptr, false);
...
void germ_qt::accounts::refresh ()
{
    model->removeRows (0, model->rowCount ());
    germ::transaction transaction (wallet.wallet_m->store.environment, nullptr, false);
    QBrush brush;
...
germ_qt::import::import (germ_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
...
germ_qt::history::history (germ::ledger & ledger_a, germ::account const & account_a, germ_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
model (new QStandardItemModel),
view (new QTableView),
...
namespace
{
class short_text_visitor : public germ::block_visitor
{
public:
    short_text_visitor (MDB_txn * transaction_a, germ::ledger & ledger_a) :
...
void germ_qt::history::refresh ()
{
    germ::transaction transaction (ledger.store.environment, nullptr, false);
    model->removeRows (0, model->rowCount ());
...
germ_qt::block_viewer::block_viewer (germ_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
...
wallet (wallet_a)
{
    ...
    QObject::connect (retrieve, &QPushButton::released, [this]() {
        ...
        {
            germ::transaction transaction (this->wallet.node.store.environment, nullptr, false);
...
void germ_qt::block_viewer::rebroadcast_action (germ::uint256_union const & hash_a)
{
    auto done (true);
    germ::transaction transaction (wallet.node.ledger.store.environment, nullptr, false);
...
germ_qt::account_viewer::account_viewer (germ_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
...
germ_qt::stats_viewer::stats_viewer (germ_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
model (new QStandardItemModel),
view (new QTableView),
...
void germ_qt::stats_viewer::refresh_stats ()
{
    model->removeRows (0, model->rowCount ());
...
germ_qt::status::status (germ_qt::wallet & wallet_a) :
wallet (wallet_a)
{
    wallet.status->setToolTip ("Wallet status, block count (blocks downloaded)");
    active.insert (germ_qt::status_types::nominal);
    set_text ();
}

void germ_qt::status::erase (germ_qt::status_types status_a)
{
    assert (status_a != germ_qt::status_types::nominal);
    auto erased (active.erase (status_a));
    (void)erased;
    set_text ();
}

void germ_qt::status::insert (germ_qt::status_types status_a)
{
    assert (status_a != germ_qt::status_types::nominal);
...
std::string germ_qt::status::text ()
{
    ...
    {
        germ::transaction transaction (wallet.wallet_m->node.store.environment, nullptr, false);
    ...
    switch (*active.begin ())
    {
        case germ_qt::status_types::disconnected:
            result = "Status: Disconnected";
            break;
        case germ_qt::status_types::working:
            result = "Status: Generating proof of work";
            break;
        case germ_qt::status_types::synchronizing:
            result = "Status: Synchronizing";
            break;
        case germ_qt::status_types::locked:
            result = "Status: Wallet locked";
            break;
        case germ_qt::status_types::vulnerable:
            result = "Status: Wallet password empty";
            break;
        case germ_qt::status_types::active:
            result = "Status: Wallet active";
            break;
        case germ_qt::status_types::nominal:
...
std::string germ_qt::status::color ()
{
    assert (!active.empty ());
    std::string result;
    switch (*active.begin ())
...
germ_qt::wallet::wallet (QApplication & application_a, germ_qt::eventloop_processor & processor_a, germ::node & node_a, std::shared_ptr<germ::wallet> wallet_a, germ::account & account_a) :
rendering_ratio (germ::Mxrb_ratio),
node (node_a),
wallet_m (wallet_a),
account (account_a),
...
active_status (*this)
{
    ...
    send_account->setPlaceholderText (germ::zero_key.pub.to_account ().c_str ());
...
void germ_qt::wallet::start ()
{
    std::weak_ptr<germ_qt::wallet> this_w (shared_from_this ());
    ...
    QObject::connect (send_blocks_send, &QPushButton::released, [this_w]() {
        if (auto this_l = this_w.lock ())
        {
            ...
            {
                germ::uint128_t actual (amount.number () * this_l->rendering_ratio);
                if (actual / this_l->rendering_ratio == amount.number ())
                {
                    ...
                    if (!parse_error)
                    {
                        ...
                        if (actual <= balance)
                        {
                            germ::transaction transaction (this_l->wallet_m->store.environment, nullptr, false);
                            if (this_l->wallet_m->store.valid_password (transaction))
                            {
                                ...
                                this_l->node.background ([this_w, account_l, actual]() {
                                    if (auto this_l = this_w.lock ())
                                    {
                                        this_l->wallet_m->send_async (this_l->account, account_l, actual, [this_w](std::shared_ptr<germ::block> block_a) {
                                            if (auto this_l = this_w.lock ())
                                            {
    ...
    node.observers.blocks.add ([this_w](std::shared_ptr<germ::block> block_a, germ::account const & account_a, germ::uint128_t const & amount_a, bool) {
        if (auto this_l = this_w.lock ())
        {
    ...
    node.observers.account_balance.add ([this_w](germ::account const & account_a, bool is_pending) {
        if (auto this_l = this_w.lock ())
        {
        ...
        if (auto this_l = this_w.lock ())
        {
            this_l->application.postEvent (&this_l->processor, new eventloop_event ([this_w, active_a]() {
                if (auto this_l = this_w.lock ())
                {
                    if (active_a)
                    {
                        this_l->active_status.insert (germ_qt::status_types::active);
                    }
                    else
                    {
                        this_l->active_status.erase (germ_qt::status_types::active);
                    }
                }
            }));
        }
    });
    node.observers.endpoint.add ([this_w](germ::endpoint const &) {
        if (auto this_l = this_w.lock ())
        {
            this_l->application.postEvent (&this_l->processor, new eventloop_event ([this_w]() {
                if (auto this_l = this_w.lock ())
        ...
        if (auto this_l = this_w.lock ())
        {
            this_l->application.postEvent (&this_l->processor, new eventloop_event ([this_w, active_a]() {
                if (auto this_l = this_w.lock ())
                {
                    if (active_a)
                    {
                        this_l->active_status.insert (germ_qt::status_types::synchronizing);
                    }
                    else
                    {
                        this_l->active_status.erase (germ_qt::status_types::synchronizing);
                    }
                }
            }));
        }
        ...
        if (auto this_l = this_w.lock ())
        {
            this_l->application.postEvent (&this_l->processor, new eventloop_event ([this_w, working]() {
                if (auto this_l = this_w.lock ())
                {
                    if (working)
                    {
                        this_l->active_status.insert (germ_qt::status_types::working);
                    }
                    else
                    {
                        this_l->active_status.erase (germ_qt::status_types::working);
                    }
                }
            }));
        }
...
void germ_qt::wallet::refresh ()
{
    {
        germ::transaction transaction (wallet_m->store.environment, nullptr, false);
...
void germ_qt::wallet::update_connected ()
{
    if (node.peers.empty ())
    {
        active_status.insert (germ_qt::status_types::disconnected);
    }
    else
    {
        active_status.erase (germ_qt::status_types::disconnected);
    }
}

void germ_qt::wallet::empty_password ()
{
...
void germ_qt::wallet::change_rendering_ratio (germ::uint128_t const & rendering_ratio_a)
{
...
std::string germ_qt::wallet::format_balance (germ::uint128_t const & balance) const
{
    auto balance_str = germ::amount (balance).format_balance (rendering_ratio, 0, false);
    auto unit = std::string ("XRB");
    if (rendering_ratio == germ::kxrb_ratio)
    {
        unit = std::string ("kxrb");
    }
    else if (rendering_ratio == germ::xrb_ratio)
    {
        unit = std::string ("xrb");
    }
...
void germ_qt::wallet::push_main_stack (QWidget * widget_a)
{
    main_stack->addWidget (widget_a);
    main_stack->setCurrentIndex (main_stack->count () - 1);
}

void germ_qt::wallet::pop_main_stack ()
{
    main_stack->removeWidget (main_stack->currentWidget ());
}

germ_qt::settings::settings (germ_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
password (new QLineEdit),
...
wallet (wallet_a)
{
    ...
    new_representative->setPlaceholderText (germ::zero_key.pub.to_account ().c_str ());
    ...
    QObject::connect (change_rep, &QPushButton::released, [this]() {
        ...
        if (!representative_l.decode_account (new_representative->text ().toStdString ()))
        {
            ...
            if (this->wallet.wallet_m->store.valid_password (transaction))
            {
                ...
                {
                    germ::transaction transaction_l (this->wallet.wallet_m->store.environment, nullptr, true);
...
void germ_qt::settings::refresh_representative ()
{
    germ::transaction transaction (this->wallet.wallet_m->node.store.environment, nullptr, false);
...
void germ_qt::settings::activate ()
{
    this->wallet.push_main_stack (window);
}

void germ_qt::settings::update_locked (bool invalid, bool vulnerable)
{
    if (invalid)
    {
        this->wallet.active_status.insert (germ_qt::status_types::locked);
    }
    else
    {
        this->wallet.active_status.erase (germ_qt::status_types::locked);
    }
    if (vulnerable)
    {
        this->wallet.active_status.insert (germ_qt::status_types::vulnerable);
    }
    else
    {
        this->wallet.active_status.erase (germ_qt::status_types::vulnerable);
    }
}

germ_qt::advanced_actions::advanced_actions (germ_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
...
wallet (wallet_a)
{
    ...
    QObject::connect (mrai, &QRadioButton::toggled, [this]() {
        if (mrai->isChecked ())
        {
            this->wallet.change_rendering_ratio (germ::Mxrb_ratio);
        }
    });
    QObject::connect (krai, &QRadioButton::toggled, [this]() {
        if (krai->isChecked ())
        {
            this->wallet.change_rendering_ratio (germ::kxrb_ratio);
        }
    });
    QObject::connect (src, &QRadioButton::toggled, [this]() {
        if (src->isChecked ())
        {
            this->wallet.change_rendering_ratio (germ::xrb_ratio);
        }
    });
    mrai->click ();
    ...
    QObject::connect (peers_bootstrap, &QPushButton::released, [this]() {
        ...
        auto error (germ::parse_endpoint (bootstrap_line->text ().toStdString (), endpoint));
        if (!error)
        {
...
void germ_qt::advanced_actions::refresh_peers ()
{
...
void germ_qt::advanced_actions::refresh_ledger ()
{
    ledger_model->removeRows (0, ledger_model->rowCount ());
    germ::transaction transaction (wallet.node.store.environment, nullptr, false);
    for (auto i (wallet.node.ledger.store.latest_begin (transaction)), j (wallet.node.ledger.store.latest_end ()); i != j; ++i)
    {
        QList<QStandardItem *> items;
        items.push_back (new QStandardItem (QString (germ::block_hash (i->first.uint256 ()).to_account ().c_str ())));
        germ::account_info info (i->second);
        std::string balance;
        germ::amount (info.balance.number () / wallet.rendering_ratio).encode_dec (balance);
...
void germ_qt::advanced_actions::refresh_stats ()
{
    wallet.stats_viewer.refresh_stats ();
}

germ_qt::block_entry::block_entry (germ_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
block (new QPlainTextEdit),
status (new QLabel),
...
wallet (wallet_a)
{
    ...
    QObject::connect (process, &QPushButton::released, [this]() {
        ...
        try
        {
            ...
            auto block_l (germ::deserialize_block_json (tree));
            if (block_l != nullptr)
            {
...
germ_qt::block_creation::block_creation (germ_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
...
void germ_qt::block_creation::deactivate_all ()
{
    account_label->hide ();
    account->hide ();
    source_label->hide ();
...
void germ_qt::block_creation::activate_send ()
{
    account_label->show ();
    account->show ();
    amount_label->show ();
...
void germ_qt::block_creation::activate_receive ()
{
    source_label->show ();
    source->show ();
}

void germ_qt::block_creation::activate_open ()
{
    source_label->show ();
    source->show ();
...
void germ_qt::block_creation::activate_change ()
{
    account_label->show ();
    account->show ();
    representative_label->show ();
            ...
            if (!error)
            {
                ...
                if (!wallet.wallet_m->store.fetch (transaction, account_l, key))
                {
                    ...
                    if (amount_l.number () <= balance)
                    {
                        ...
                        germ::state_block send (account_l, info.head, rep_block->representative (), balance - amount_l.number (), destination_l, key, account_l, 0);
...
void germ_qt::block_creation::create_receive ()
{
    germ::block_hash source_l;
    ...
    if (!error)
    {
        ...
        if (block_l != nullptr)
        {
            ...
            {
                germ::pending_key pending_key (destination, source_l);
                ...
                if (!wallet.node.store.pending_get (transaction, pending_key, pending))
                {
                    ...
                    if (!error)
                    {
                        ...
                        if (!error)
                        {
                            auto rep_block (wallet.node.store.block_get (transaction, info.rep_block));
                            assert (rep_block != nullptr);
                            germ::state_block receive (pending_key.account, info.head, rep_block->representative (), info.balance.number () + pending.amount.number (), source_l, key, pending_key.account, 0);
...
void germ_qt::block_creation::create_change ()
{
    germ::account account_l;
    auto error (account_l.decode_account (account->text ().toStdString ()));
    if (!error)
    {
        ...
        if (!error)
        {
            ...
            if (!error)
            {
                ...
                if (!error)
                {
                    germ::state_block change (account_l, info.head, representative_l, info.balance, 0, key, account_l, 0);
...
void germ_qt::block_creation::create_open ()
{
    germ::block_hash source_l;
    auto error (source_l.decode_hex (source->text ().toStdString ()));
    if (!error)
                ...
                if (!destination.is_zero ())
                {
                    ...
                    if (!wallet.node.store.pending_get (transaction, pending_key, pending))
                    {
                        ...
                        if (error)
                        {
                            ...
                            if (!error)
                            {
                                germ::state_block open (pending_key.account, 0, representative_l, pending.amount, source_l, key, pending_key.account, 0);
