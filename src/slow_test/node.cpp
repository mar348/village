...
TEST (system, receive_while_synchronizing)
{
	std::vector<std::thread> threads;
	{
		...
		system.alarm.add (std::chrono::steady_clock::now () + std::chrono::milliseconds (200), ([&system, &key]() {
			auto hash (system.wallet (0)->send_sync (germ::test_genesis_key.pub, key.pub, system.nodes[0]->config.receive_minimum.number ()));
			auto block (system.nodes[0]->store.block_get (germ::transaction (system.nodes[0]->store.environment, nullptr, false), hash));
...
TEST (ledger, deep_account_compute)
{
	...
	germ::keypair key;
	auto balance (germ::genesis_amount - 1);
	germ::send_block send (genesis.hash (), key.pub, balance, germ::test_genesis_key.prv, germ::test_genesis_key.pub, 0);
	ASSERT_EQ (germ::process_result::progress, ledger.process (transaction, send).code);
	germ::open_block open (send.hash (), germ::test_genesis_key.pub, key.pub, key.prv, key.pub, 0);
	...
	{
		balance -= 1;
		germ::send_block send (sprevious, key.pub, balance, germ::test_genesis_key.prv, germ::test_genesis_key.pub, 0);
		ASSERT_EQ (germ::process_result::progress, ledger.process (transaction, send).code);
		sprevious = send.hash ();
		germ::receive_block receive (rprevious, send.hash (), key.prv, key.pub, 0);
...
TEST (wallet, multithreaded_send)
{
	std::vector<std::thread> threads;
	{
		...
		wallet_l->insert_adhoc (germ::test_genesis_key.prv);
		for (auto i (0); i < 20; ++i)
		{
			threads.push_back (std::thread ([wallet_l, &key]() {
				for (auto i (0); i < 1000; ++i)
				{
					wallet_l->send_action (germ::test_genesis_key.pub, key.pub, 1000);
				}
			}));
		}
		while (system.nodes[0]->balance (germ::test_genesis_key.pub) != (germ::genesis_amount - 20 * 1000 * 1000))
		{
			system.poll ();
		}
	}
		...
		threads.push_back (std::thread ([&system]() {
			for (auto i (0); i != 1000; ++i)
			{
				...
				for (auto j (0); j != 10; ++j)
				{
					...
					system.nodes[0]->store.account_put (transaction, hash, germ::account_info ());
				}
			}
		}));
	}
...
{
	germ::system system (24000, 64);
	system.wallet (0)->insert_adhoc (germ::test_genesis_key.prv);
	auto previous (system.nodes[0]->latest (germ::test_genesis_key.pub));
	auto balance (system.nodes[0]->balance (germ::test_genesis_key.pub));
	ASSERT_FALSE (previous.is_zero ());
	for (auto j (0); j != system.nodes.size (); ++j)
	{
		balance -= 1;
		germ::keypair key;
		germ::send_block send (previous, key.pub, balance, germ::test_genesis_key.prv, germ::test_genesis_key.pub, 0);
		previous = send.hash ();
		for (auto i (0); i != system.nodes.size (); ++i)
		{
			auto send_result (system.nodes[i]->process (send));
			ASSERT_EQ (germ::process_result::progress, send_result.code);
			germ::keypair rep;
			auto open (std::make_shared<germ::open_block> (previous, rep.pub, key.pub, key.prv, key.pub, 0));
			system.nodes[i]->work_generate_blocking (*open);
			auto open_result (system.nodes[i]->process (*open));
			ASSERT_EQ (germ::process_result::progress, open_result.code);
			germ::transaction transaction (system.nodes[i]->store.environment, nullptr, false);
	...
	while (again)
	{
		empty = 0;
		single = 0;
		std::for_each (system.nodes.begin (), system.nodes.end (), [&](std::shared_ptr<germ::node> const & node_a) {
		...
		for (auto i (nodes.begin ()), n (nodes.end ()); i != n; ++i)
		{
			switch (*i)
			{
				...
				case 1:
					...
					{
						++message_count;
						auto entry (germ::random_pool.GenerateWord32 (0, node_count - 1));
						switch (nodes[entry])
						{
							case 0:
								nodes[entry] = 1;
...
TEST (peer_container, random_set)
{
	...
	germ::peer_container container (germ::endpoint (loopback, 24000));
	for (auto i (0); i < 200; ++i)
	{
		container.contacted (germ::endpoint (loopback, 24001 + i), 0);
	}
...
TEST (store, unchecked_load)
{
	...
	auto block (std::make_shared<germ::send_block> (0, 0, 0, germ::test_genesis_key.prv, germ::test_genesis_key.pub, 0));
...
TEST (store, vote_load)
{
	...
	for (auto i (0); i < 1000000; ++i)
	{
		auto vote (std::make_shared<germ::vote> (germ::test_genesis_key.pub, germ::test_genesis_key.prv, i, block));
